...
@section('content')
<h2 class="mb-3">Edit Ticket — <code>{{ $tickets->ticket_number }}</code></h2>
<div class="card shadow-sm border-0">
    <div class="card-body">
        <form method="POST" action="{{ route('tickets.update', $tickets) }}" id="ticketForm">
            @csrf
            @method('PUT')
            @include('tickets._form')
            <button type="submit" class="btn btn-primary" id="submitBtn">
                <span class="spinner-border spinner-border-sm d-none" id="spinner" role="status"></span>
                <span id="btnText">Update Ticket</span>
            </button>
        ...
        form.on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    // redirect back to the tickets list after a successful update
                    window.location.href = "{{ route('tickets.index') }}";
                    return; // navigating away; skip button reset
                },
                error: function(xhr) {
                    if (xhr.status === 422) {
